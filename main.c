...
#ifdef S_IRWXU
        int example_fd = open("randomfiletrustme", O_CREAT | O_RDWR, S_IRWXU);
#else
        int example_fd = open("randomfiletrustme", O_CREAT | O_RDWR, 0);
#endif
        if (example_fd == -1) {
...
int main() {
    ...
    test_unix_dgram(AF_UNIX, SOCK_DGRAM);
